...
developing an accurate KNNJoin algorithm for spark framework, which is
...
Theorem1: \cite{lu_efficient_2012} Given two pivots $s_i^p$, $s_j^p$ of two voronoi
partition $S_i$ and $S_j$, Let HP($s_i^p$, $s_j^p$) be a generalized
hyperplane which is equidistant from $s_i^p$ and $s_j^p$. Then $\forall\ o\
\epsilon\ S_j$

\medskip

$d(o, HP(s_i^p, s_j^p))$ = $\frac{|o,s_i^p|^2 - |o,s_j^p|^2}{2 x |s_i^p, s_j^p|}$

\bigskip

Corollary1: Given a point $o\ \epsilon\ S_j$, $\theta = \max{d(o, KNN(o, S_j))}$ be max K Nearest
neighbour distance from o in partition $S_j$ then partition
$S_i$ can contain nearest neighbour only if

\medskip

$d(o, HP(s_i^p, s_j^p)) > \theta $
...
\FORALL{$r$ in $R_i$}
\STATE $r.nearest\_partition \leftarrow j$  where
$d(r, HP(s_i^p, s_j^p)) > \theta\ and\ j\ =\ 1\ to\ N$
\ENDFOR

\STATE Move r to $first(r.nearest\_partition)\ \forall\ r\ in\ R_i$
\REPEAT
\STATE Find KNN($R_i$,$S_i$)
\FORALL{$r$ in $R_i$}
\STATE $r.nearest\_partition \leftarrow R_j$  where
$d(r, HP(s_i^p, s_j^p)) > \theta\ and\ j\ in\ r.nearest\_partition$
\ENDFOR
\UNTIL{r.nearest\_partition is none $\forall\ r\ in\ R$}
